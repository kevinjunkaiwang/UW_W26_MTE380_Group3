...
\noindent\textbf{Purpose of document:} record the fuzzy scheduling design calculation used by the line-following controller and document the assumptions, equations, and outputs used for implementation.
\vspace{\baselineskip}

\noindent\textbf{Purpose of calculation:} compute an adaptive control mode from measured operating conditions so the robot can run fast when visibility is good and remain stable when visibility is poor.
\vspace{\baselineskip}

\noindent\textbf{Result of calculation:} two normalized inputs, $X_1$ (speed) and $X_2$ (visible line length), are mapped through fuzzy rules to $X^*$ and a winning label that selects $(v_{\max}, K_p, K_i, K_d)$.
\vspace{\baselineskip}

\noindent\textbf{How this supports design decision:} the team uses adaptive scheduling (instead of fixed gains) as the primary software control strategy because the calculation directly links sensing quality to speed and controller aggressiveness.

\section{Objective / Purpose Statement}
\noindent\textbf{Design objective:} maximize line-following speed while keeping lateral error stable and avoiding loss of the line.
\vspace{\baselineskip}

\noindent\textbf{Design constraint:} motor speed is capped by hardware and the camera can only observe a limited lookahead length.
\vspace{\baselineskip}

\noindent\textbf{Design decision:} use a fuzzy scheduler that maps the current speed command and the visible line length into a speed cap and PID gains.
...
\midrule
Normalized speed input & $X_1$ & 0 to 100 \\
Visible line length input & $X_2$ & 0 to 100 \\
Defuzzified scheduler output & $X^*$ & 1 to 100 (clamped) \\
Camera ROI bottom fraction & \texttt{roi\_bottom\_ratio} & 0.60 (default) \\
Row occupancy threshold & \texttt{row\_occupancy\_frac} & 0.02 (default) \\
Wheel command bounds & $u_L, u_R$ & -1 to 1 \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection*{Underlying Engineering Principle}
The underlying engineering principle is feedback control of a dynamic system with gain scheduling. The IR array provides lateral error for closed-loop PID correction, while camera-derived line visibility provides a lookahead proxy for available path information. Fuzzy inference approximates a nonlinear scheduling law that improves robustness compared with a single fixed-gain controller.
